...
for wid in next_ids:
  ...
  # Get result count
  response_data = call_api(url=f'{oal_domain}works?mailto={my_email}&api_key={my_key}&per-page=1&page=1&select=id&filter={direction}:{wid}')
  ...
  if cit_count > 0:
    ...
    while cursor is not None:
      ...
      response_data = call_api(url=f'{oal_domain}works?mailto={my_email}&api_key={my_key}&per-page={ppg}&cursor={cursor}&select={",".join(fields_to_return)}&filter={direction}:{wid}')
